...
flextable::regulartable(mod18.2c_2024$agedbase %>%
  filter(Fleet ==1 & sex == 1 & Yr > 2000 & !(Yr %in% unique(mod18.2c_2024$ghostlendbase$Yr[mod18.2c_2024$ghostagedbase$Fleet==1]))) %>%
    mutate(effN = round(effN,1),
           Obs = round(Obs, 4)) %>%
...
flextable::regulartable(mod18.2c_2024$agedbase %>%
  filter(Fleet ==1 & sex == 2 & Yr > 2000 & !(Yr %in% unique(mod18.2c_2024$ghostlendbase$Yr[mod18.2c_2024$ghostagedbase$Fleet==1]))) %>%
    mutate(effN = round(effN,1),
           Obs = round(Obs, 4)) %>%
                          select(Yr, effN,'Age' = Bin, Obs ) %>%  
                          tidyr::pivot_wider(
