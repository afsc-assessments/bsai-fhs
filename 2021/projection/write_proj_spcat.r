  ...
  write_proj_spcat<-function(dir="foo",sdir="data/",data_file="Model_Proj_spcat.dat",data=Models[[1]],ct_yrs=2)
  {
	  ...
	  write(noquote("#	data	files	for	each	species"),file=write_file,append=TRUE)
	  write(noquote("data/Model_Proj.dat"),file=write_file,append=TRUE)																				
	  ...
	  if(ct_yrs>1)
	  {
	  	for(i in 2:ct_yrs)
	  	{
	  		write(paste(LY+(i-1), sum(data$catch$Obs[data$catch$Yr==(LY+(i-1))])),file=write_file,append=TRUE)
	  	}
	  }
