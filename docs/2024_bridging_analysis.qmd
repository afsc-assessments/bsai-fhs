...
We explored using the `surveyISS` r package to generate updated input sample sizes for the *survey* compositional data (code [here](https://github.com/afsc-assessments/bsai-fhs/blob/main/2024/R/updated_survey_iss.R)).
...
We then bridged in the new sample sizes into model 18.2c (2024) and re-tuned the Francis weights. There are some minor differences between the 'base' 18.2c (2024) model shown below and the figures above because as of this writing (Oct 2024) the base has been updated with 2024 survey data, survey length comps, and a better estimate of 2024 catch.
...
## Tuning procedures: only did this once
# mod03_00 <- SS_output(here::here(year, 'model_runs','02_bridging','03.00_18.4c_2024-lengthSurveyISS'))
# mod03_01 <- SS_output(here::here(year, 'model_runs','02_bridging','03.01_18.4c_2024-caalSurveyISS'))
# mod03_02 <- SS_output(here::here(year, 'model_runs','02_bridging','03.02_18.4c_2024-allSurveyISS'))
# # copy in mod03_02, set weights to 1 and run for Francis prep
# mod03_02x <- SS_output(here::here(year, 'model_runs','02_bridging','03.03_18.4c_2024-allSurveyISS-francis'))

## do three rounds of Francis tuning on this model, then copy in the suggested tunings
# r4ss::tune_comps(replist = mod03_02x, option = 'Francis',
#                  digits = 6, niters_tuning = 3, exe = 'ss',
#                  write = TRUE, dir = here::here(year, 'model_runs','02_bridging','03.03_18.4c_2024-allSurveyISS-francis'))
# rm(mod03_02x)
# mod03_03 <- SS_output(here::here(year, 'model_runs','02_bridging','03.03_18.4c_2024-allSurveyISS-francis'))

# ss_compare <- SSsummarize(list(mod18.2c_2024,
#                                mod03_00,
#                                mod03_01,
#                                mod03_02,
#                                mod03_03))
# 
# SSplotComparisons(ss_compare,
#                   print = T,
#                   plotdir = here('docs',year,'02_surveyiss_bridging'), legendlabels = c('Model 18.2c (2024)',  "surveyISS length only",  "surveyISS CAAL only",  "surveyISS length and CAAL",  "surveyISS length and CAAL + francis weights"), col = c('gold','grey20','grey40','grey60','blue'))
...
data.frame(Fleet = rep(c('Fishery','Survey'),2),
      Type = c('Length','Length','Age','CAAL'),
     Base_Value = c(0.079732,0.359831,0.086647,0.272054),
    Survey_ISS_value = c(0.079018,0.062439, 0.085811,0.481957))
```
...
```{r,  include=F, eval = TRUE, echo = FALSE, warning = FALSE, message =FALSE}
mod18.2c_2024_nhaul <- SS_output(here::here(year,'mgmt','18.2c_2024-nhauls'), verbose = FALSE, hidewarn = TRUE, warn = FALSE)
```
...
```{r,  include=T, eval = TRUE, echo = FALSE, warning = FALSE, message =FALSE}
  ...
  rbind(
mod18.2c_2024_nhaul$likelihoods_used %>% mutate(src = 'nhauls',
                                                variable = row.names(.)), mod18.2c_2024_nhaul$likelihoods_used %>% mutate(src = 'surveyISS', variable = row.names(.))) %>%
    filter(values > 0) %>%
    tidyr::pivot_wider(names_from = src, values_from = values, id_cols = variable) %>% kableExtra::kable()
```
