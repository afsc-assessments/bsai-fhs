...
The old datapull script and `afscdata` both read from `norpac.debriefed_spcomp`, though one reads from `mv` and the other doesn't
...
*I was unable to reproduce the lengthy age-comp routine* used for the older version of the model; for reproducibility purposes, I am going to move towards the `afscdata` approach, which requires about 10% of the code to run. Importantly, I tested Model 18.2c with *only* the age-comp data changed to those available using my new approach and found the impact on derived quantities to be trivial.

See the comments on the figures below for more information.

There are some inconsistencies between the `afscdata` and old data pull approaches. Specifically, the old approach resulted in a duplicated vector of integer age-frequencies (for males & females); it appears that he number of hauls 
...
SSplotComparisons(SSsummarize(list(mod_2020,mod_2020_newcomps)), 
