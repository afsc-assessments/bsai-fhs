...
*I could not completely reproduce the 2020 Model's* method of pulling these data from scratch because the schema (`haehnr`) that it calls from has been deleted. The workup on the `AL.df` object corresponds to `2020/data/ages_survey_ebs.csv` which is the roughly the same as what comes out of `_specimen`. I first tried to calculate the CAALs starting from the raw data using an intuitive aprpoach, but returned very different datasets. Instead, I consolidated the code used in the last model into `R/calculate_survey_CAALs.R`

In addition, the `afscassess` functions aren't yet ready to handle the BSAI survey data, so this had to be done with my own functions in `R/`.
...
-   Pull the raw data from `racebase` using the `afscdata` approach (see `2024_analysis.R`)
-   Calculate the marginal survey ages from `bsai_length_specimen.csv`.
-   Calculate the marginal survey lengths from `bsai_length_data.csv`.
-   Calculate conditional ages-at-length for each sex via the following: Starting with the EBS ages, filter out Bering Flounder (code `10140`) and only include the EBS (subarea \<= 6) and sexed fish (3). Bin the data into length and age bins greater than 0 (legacy code `BIN_AGE_DATA` and `BIN_LEN_DATA`). Group by bins and tabulate the number in each unique combination; filling missing values from zero. Reformat into SS3 syntax.
...
```{r, include=T, echo = FALSE, warning = FALSE, fig.cap= "Only trivial differences in this historical data provided to the original assessment (shown here) vs. those pulled using a combination of afscdata and the legacy code (next tab)." }
SSplotComps(mod_2020,fleets=2, subplots = 3, kind = 'cond', 
            datonly = TRUE, maxrows = 6,showsampsize = TRUE, showeffN = FALSE)
```

#### Survey CAALs using `afscdata`

```{r, include=T, echo = FALSE, warning = FALSE, fig.cap="Only trivial differences in this historical data provided to the original assessment (previous tab) vs. those pulled using a combination of afscdata and the legacy code (shown here)." }
